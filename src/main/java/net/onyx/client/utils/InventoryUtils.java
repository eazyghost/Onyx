...
import net.onyx.client.interfaces.mixin.IClientPlayerInteractionManager;
...
public class InventoryUtils {
    ...
    public static boolean nameContains(String contains, Hand hand) {
        ItemStack item = OnyxClient.getClient().player.getStackInHand(hand);
    ...
    public static void moveItem(int from, int to) {
        ...
        boolean wasEmpty = OnyxClient.me().getInventory().getStack(to).isEmpty();
    ...
    public static int getFreeSlot() {
        int slot = OnyxClient.me().getInventory().getEmptySlot();
    ...
    public static class Action {
        ...
        private void run() {
            boolean hadEmptyCursor = OnyxClient.getClient().player.currentScreenHandler.getCursorStack().isEmpty();
            ...
            if (!isRecursive && hadEmptyCursor && preType == SlotActionType.PICKUP && preTwo && (preFrom != -1 && preTo != -1) && !OnyxClient.getClient().player.currentScreenHandler.getCursorStack().isEmpty()) {
                isRecursive = true;
        ...
        private void click(int id) {
            OnyxClient.getClient().interactionManager.clickSlot(OnyxClient.getClient().player.currentScreenHandler.syncId, id, data, type, OnyxClient.getClient().player);
        }
    }
    ...
    public static boolean swap(int slot, boolean swapBack) {
        ...
        if (swapBack && previousSlot == -1) previousSlot = OnyxClient.getClient().player.getInventory().selectedSlot;

        OnyxClient.getClient().player.getInventory().selectedSlot = slot;
        ((IClientPlayerInteractionManager) OnyxClient.getClient().interactionManager).syncSelected();
        return true;
    }
