...
public class BlockUtils {
	private static final MinecraftClient client = OnyxClient.getClient();
	...
	public static ActionResult interact(Vec3d vec3d, Direction dir) {
		...
		return OnyxClient.getClient().interactionManager.interactBlock(OnyxClient.getClient().player, OnyxClient.getClient().player.getActiveHand(),result);
	}
	...
	public static boolean placeBlock(BlockPos pos, BlockState state) {
		...
		for (BlockPos neighbor : neighbors)
		{
			...
			BlockHitResult hit = OnyxClient.getClient().world.raycastBlock(RotationUtils.getEyesPos(), faceCenter, neighbor, BlockUtils.getBlockState(neighbor).getOutlineShape(OnyxClient.getClient().world, neighbor), BlockUtils.getBlockState(neighbor));
			if (hit == null)
			{
		...
		ActionResult result = OnyxClient.getClient().interactionManager.interactBlock(OnyxClient.getClient().player, Hand.MAIN_HAND, new BlockHitResult(faceCenterToClick, directionToClick, neighborToClick, false));
	...
	{
		return OnyxClient.getClient().world.canPlace(state, pos, null);
	}
