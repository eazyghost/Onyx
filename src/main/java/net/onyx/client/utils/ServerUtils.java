...
public class ServerUtils {
    ...
    public static void connectToServer(ServerInfo info, Screen prevScreen) {
        ...
        ConnectScreen.connect(prevScreen, OnyxClient.getClient(), ServerAddress.parse(info.address), info);
    }

    public static AbstractClientPlayerEntity getPlayerByName(String name) {
        List<AbstractClientPlayerEntity> players = OnyxClient.getClient().world.getPlayers();
    ...
    public static Integer getTotalOnlinePlayers() {
        if (OnyxClient.getClient().world == null) return -1;

        return OnyxClient.getClient().world.getPlayers().size();
    }
