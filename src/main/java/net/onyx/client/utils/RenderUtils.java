...
import net.onyx.client.interfaces.mixin.IItemRenderer;
...
public class RenderUtils {
	...
	public static Vec3d getEntityRenderPosition(Entity entity, double partial) {
		double x = entity.prevX + ((entity.getX() - entity.prevX) * partial) - OnyxClient.getClient().getEntityRenderDispatcher().camera.getPos().x;
		double y = entity.prevY + ((entity.getY() - entity.prevY) * partial) - OnyxClient.getClient().getEntityRenderDispatcher().camera.getPos().y;
		double z = entity.prevZ + ((entity.getZ() - entity.prevZ) * partial) - OnyxClient.getClient().getEntityRenderDispatcher().camera.getPos().z;
	...
	private static Vec3d to2D(double x, double y, double z) {
		int displayHeight = OnyxClient.getClient().getWindow().getHeight();
	...
	public static double getScaleFactor() {
		return OnyxClient.getClient().getWindow().getScaleFactor();
	}
	...
	public static Frustum getFrustum() {
		return ((WorldRendererAccessor) OnyxClient.getClient().worldRenderer).getFrustum();
	}
	...
	public static MatrixStack matrixFrom(double x, double y, double z) {
		...
		Camera camera = OnyxClient.getClient().gameRenderer.getCamera();
	...
	public static void renderBox(Entity e, double partialTicks, MatrixStack mStack, boolean blend, float extraSize) {
        ...
        float f = OnyxClient.me().distanceTo(e) / 20F;
	...
	public static void scissorBox(int startX, int startY, int endX, int endY)
	{
		...
		int bottomY = OnyxClient.getClient().currentScreen.height - endY;
		double factor = OnyxClient.getClient().getWindow().getScaleFactor();
	...
	public static Vec3d getCameraPos(float delta) {
		Entity ent = OnyxClient.getClient().cameraEntity;
	...
	public static Vec3d getCameraPos() {
		return OnyxClient.getClient().getBlockEntityRenderDispatcher().camera.getPos();
	}
	
	public static BlockPos getCameraBlockPos() {
		return OnyxClient.getClient().getBlockEntityRenderDispatcher().camera
			.getBlockPos();
	}
	...
	public static void renderGuiItemOverlay(TextRenderer renderer, ItemStack stack, float x, float y, float scale, @Nullable String countLabel) {
		if (!stack.isEmpty()) {
			...
			if (stack.getCount() != 1 || countLabel != null) {
				...
				matrixStack.translate(0.0D, 0.0D, OnyxClient.getClient().getItemRenderer().zOffset + 200.0F);
				VertexConsumerProvider.Immediate immediate = VertexConsumerProvider.immediate(Tessellator.getInstance().getBuffer());
				renderer.draw(string, x + 19 - 2 - renderer.getWidth(string), y + 6 + 3, 16777215, true, matrixStack.peek().getPositionMatrix(), immediate, false, 0, 15728880);
				immediate.draw();
			}
			...
			ClientPlayerEntity clientPlayerEntity = OnyxClient.getClient().player;
	...
	public static void drawItem(ItemStack itemStack, int x, int y, double scale, boolean overlay) {
		...
		OnyxClient.getClient().getItemRenderer().renderGuiItemIcon(itemStack, (int) (x / scale), (int) (y / scale));
		if (overlay) OnyxClient.getClient().getItemRenderer().renderGuiItemOverlay(OnyxClient.getClient().textRenderer, itemStack, (int) (x / scale), (int) (y / scale), null);
	...
	public static void drawItem(ItemStack stack, float xPosition, float yPosition, float scale) {
		...
		IItemRenderer iItemRenderer = (IItemRenderer) OnyxClient.getClient().getItemRenderer();
		iItemRenderer.renderItemIntoGUI(stack, xPosition, yPosition, scale);
		renderGuiItemOverlay(OnyxClient.getClient().textRenderer, stack, xPosition - 0.5f, yPosition + 1, scale, amountText);
	}
	...
	public static void drawString(String string, int x, int y, int color, float scale)
	{
		...
		VertexConsumerProvider.Immediate immediate = VertexConsumerProvider.immediate(Tessellator.getInstance().getBuffer());
		OnyxClient.getClient().textRenderer.draw(string, (float) x / scale, (float) y / scale, color, true, matrixStack.peek().getPositionMatrix(), immediate, false, 0, 0xF000F0);
		immediate.draw();
	}
}