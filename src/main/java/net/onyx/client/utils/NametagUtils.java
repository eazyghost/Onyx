...
public class NametagUtils {
    ...
    public static void onRender(MatrixStack matrices, Matrix4f projection) {
        ...
        camera.set(OnyxClient.getClient().gameRenderer.getCamera().getPos());
        cameraNegated.set(camera);
        cameraNegated.negate();

        windowScale = OnyxClient.getClient().getWindow().calculateScaleFactor(1, false);
    }
    ...
    public static boolean to2D(Vec3 pos, double scale, boolean distanceScaling) {
        ...
        pmMat4.toScreen();
        double x = pmMat4.x * OnyxClient.getClient().getWindow().getFramebufferWidth();
        double y = pmMat4.y * OnyxClient.getClient().getWindow().getFramebufferHeight();

        if (Double.isInfinite(x) || Double.isInfinite(y)) return false;

        pos.set(x / windowScale, OnyxClient.getClient().getWindow().getFramebufferHeight() - y / windowScale, pmMat4.z);
        return true;
    }
