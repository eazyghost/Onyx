...
import net.onyx.client.components.systems.TargetCircle;
...
public class Aimbot extends Module {
    ...
    private final TargetCircle targetCircle     = new TargetCircle(new Colour(255, 255, 255, 255));
    private final TargetCircle shootCircle      = new TargetCircle(new Colour(255, 0, 0, 255));
    private final TargetCircle antiSmoothCircle = new TargetCircle(new Colour(255, 255, 0, 255));
    ...
    private Vec3d getPredictPos(Entity target, float tickDelta) {
        ...
        float extrapolationAmount = 1 * (float)(double) this.getDoubleSetting("PredictStep");
    ...
    */
    private List<Entity> getTargets() {
        ...
        Vec3d localPos = OnyxClient.me().getPos();
        int localTeamColour = this.getTeamColour(OnyxClient.me());

        // Get all of the players
        for (Entity ent : OnyxClient.getClient().world.getEntities()) {
            if (!(ent instanceof PlayerEntity)) continue;

            Entity player = ent;

            // Make sure the player is not us
            if (player == OnyxClient.me()) continue;
            ...
            boolean canSee = this.getBoolSetting("Preaim") ? ClientUtils.canSee(pos) : OnyxClient.me().canSee(player);
            if (!canSee) continue;
    ...
    private Entity getTarget() {
        ...
        OldRotation.Rotation current = new OldRotation.Rotation(OnyxClient.me().getYaw(), OnyxClient.me().getPitch());
        ...
        players.sort(new Comparator<Entity>() {
            ...
            public int compare(Entity a, Entity b) {
                ...
                OldRotation.Rotation aRotation = OldRotation.getRequiredRotation(aPos);
                OldRotation.Rotation bRotation = OldRotation.getRequiredRotation(bPos);
     ...
     */
    private final Queue<Vec3d> previousPositions = new LinkedList<>();
    ...
    private void addCurrentBacktrackPos() {
        this.addNewBacktrackPos(OldRotation.getEyePos());
    }
    ...
    private void aimbotThink(float tickDelta) {
        ...
        OldRotation.Rotation targetRotation = OldRotation.getRequiredRotation(localPos, targetPos, tickDelta);
        OldRotation.Rotation current = new OldRotation.Rotation(OnyxClient.me().getYaw(), OnyxClient.me().getPitch());
        OldRotation.Rotation diff = targetRotation.difference(current);
        ...
        OnyxClient.me().setPitch(pitch);
        OnyxClient.me().setYaw(yaw);
        ...
        if (this.getBoolSetting("AutoShoot")) {
            ...
            // Shooting delay
            double curr = OnyxClient.getCurrentTime();
