...
import net.onyx.client.components.systems.binds.impl.CommandBind;
...
import net.onyx.client.utils.ThirdPartyUtils;
...
public class Binds extends Module {
    ...
    private Boolean isChatDelimiter(Integer key) {
        ...
        return (keyChar == OnyxClient.getInstance().commandHandler.delimiter.charAt(0));
    }
    ...
    public void fireEvent(Event event) {
        switch (event.getClass().getSimpleName()) {
            case "OnKeyEvent": {
                ...
                // Handle the keypress
                if (OnyxClient.getClient().currentScreen == null) binds.fireBinds(e.key);

                // Open chat if it is our command button.
                if (this.isChatDelimiter(e.key) && OnyxClient.getClient().currentScreen == null) {
                    ClientUtils.openChatScreen();
                }

                // Open ClickGUI (if we dont have meteor)
                if (!ThirdPartyUtils.isOtherClientLoaded() && this.getBoolSetting("GUIKey") && e.key == OnyxClient.getInstance().config.menuKey) {
                    ChatUtils.hideNextChat = true;
                    OnyxClient.getInstance().getModules().get("clickgui").toggle();
                }
        ...
        if (data.containsKey("binds")) {
            ...
            for (String key : bindsObj.keySet()) {
                ...
                for (String command : commands) {
                    ...
                    if (OnyxClient.getInstance().getModules().containsKey(command)) {
                        Module module = OnyxClient.getInstance().getModules().get(command);
    ...
    private static class BindsSub extends Command {
        ...
        protected Module getModule(String command) {
            OnyxClient cc = OnyxClient.getInstance();
