...
public class TapeMeasure extends Module {
    ...
    private void handleVecDistance(BlockPos deltaVector) {
        Integer dX = Math.abs(deltaVector.getX());
        Integer dY = Math.abs(deltaVector.getY());
        Integer dZ = Math.abs(deltaVector.getZ());
    ...
    private boolean shouldRenderBlock(BlockPos pos) {
        ...
        double distanceToBlock = OnyxClient.me().getPos().distanceTo(blockVec);
        OnyxClient.me();
