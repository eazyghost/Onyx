...
import net.onyx.client.config.Flatternable;
...
import net.onyx.client.events.EventListener;
...
public abstract class Module extends Settings implements EventListener, Flatternable {
    ...
    protected final static EventManager eventManager = OnyxClient.getInstance().getEventManager();
    ...
    */
     ...
     */
    private final EventEmitter emitter = OnyxClient.getInstance().emitter;
    ...
    public void lift(HashMap<String, String> data) {
        ...
        for (String name : data.keySet()) {
            ...
            if (setting == null) {
                OnyxClient.log(String.format("Unknown setting '%s' in mod '%s.'", name, this.getName()));
                continue;
            }
            ...
            if (setting.value instanceof Mode) {
                ...
                if (!mode.setState(val)) {
                    OnyxClient.log(String.format("Invalid state '%s' for setting '%s' in mod '%s.'", val, name, this.getName()));
                    continue;
                }
