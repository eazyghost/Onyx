...
public class SkeletonESP extends Module {
    ...
    public void fireEvent(Event event) {
        switch (event.getClass().getSimpleName()) {
            ...
            case "OnRenderEvent": {
                ...
                OnyxClient.getClient().world.getEntities().forEach(entity -> {
                    if (!(entity instanceof PlayerEntity playerEntity)) return;
                    if (OnyxClient.getClient().options.getPerspective() == Perspective.FIRST_PERSON  && OnyxClient.getClient().player == entity) return;
                    ...
                    PlayerEntityRenderer livingEntityRenderer = (PlayerEntityRenderer)(LivingEntityRenderer<?, ?>) OnyxClient.getClient().getEntityRenderDispatcher().getRenderer(playerEntity);
                    ...
                    if (OnyxClient.getClient().player == entity && RotationUtils.rotationTimer < rotationHoldTicks) h = RotationUtils.serverYaw;
                    float j = MathHelper.lerpAngleDegrees(g, playerEntity.prevHeadYaw, playerEntity.headYaw);
                    if (OnyxClient.getClient().player == entity && RotationUtils.rotationTimer < rotationHoldTicks) j = RotationUtils.serverYaw;
                    ...
                    if (OnyxClient.getClient().player == entity && RotationUtils.rotationTimer < rotationHoldTicks) m = RotationUtils.serverPitch;
    ...
    private Color getColorFromDistance(Entity entity) {
        double distance = OnyxClient.getClient().gameRenderer.getCamera().getPos().distanceTo(entity.getPos());
